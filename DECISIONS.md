...
- CSV includes every payment row. `amount` is its counted contribution (zero for blocked or known failed rows); `authorizedAmount` preserves the original proposal. Sum `amount` separately by network/asset to reproduce dashboard totals. Formula-like text cells are escaped; monetary cells remain exact integer strings. Invoice export is a standalone printable HTML statement.
