...
test("paid intake enforces an exact cap without reading full history", () => {
  ...
  const meter = new Meter(
    ledger,
    parseConfig({
      budgets: { perTask: null, perAgent: null, global: { amount, window: "24h" } },
    }),
  );
  ...
  try {
    ...
    try {
      ...
      expect(meter.begin({ url, method: "GET", headers: paymentHeader(2, 21, url) }).body).toEqual({
