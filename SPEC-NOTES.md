...
| --- | --- |
| Demo placeholder (§6) | The follow-up request replaces the original placeholder with a real 20-second Windows ConPTY recording rendered by agg. The simulation, recording tools, and output stay local; no payment is signed or settled. The GIF ships in the package while capture helpers, source recording, and MP4 remain in the local recording workspace on E:. |
