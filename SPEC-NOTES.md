...
| One USDC balance and scalar maximum (§3) | Every total and budget is scoped separately by normalized network and contract. Symbol budgets match the local registry. `maxSingleAsset`, default `USDC`, scopes `maxSinglePayment`. Parsed unknown assets are denied by default; `policy.unknownAsset: "allow"` explicitly permits them, and only matching contract budgets and caps then apply. Optional budget `network` narrows a rule further. |
